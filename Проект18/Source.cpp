...
int main() {
	...
	case 3:
	{
		...
		for (auto it = as.begin(); it != as.end(); it++) {
			auto itr = bs.equal_range(*it);//первый и последний адреса значений
		...
		do {
			...
			case 3: {
				...
				*/
				for (auto it = as.begin(); it != as.end(); it++) {
					...
					for_each(itr.first, itr.second, [author](pair<string, book> e) {
						if (e.second.getAuthor() == author)
							cout << e.second << endl;
					});
			...
			case 4: {
				...
				for (auto it = as.begin(); it != as.end(); it++) {
					...
					for_each(itr.first, itr.second, [year](pair<string, book> e) {
						if (e.second.getYear() == year)
							cout << e.second << endl;
					});
